...
export default function DiscountsIndex() {
  ...
  useEffect(() => {
    setOverviewPage(1);
  }, [overviewQuery, overviewPageSize, groupedTierDiscounts.length]);
...
.disc-btn-primary:disabled {
  ...
  transform: none;
    ...
    .disc-live-widget {
      border-radius: 12px; border: 1px solid; padding: 12px; transition: all 0.15s ease;
    }
    ...
    .disc-live-widget-row { display: grid; grid-template-columns: 88px 1fr 88px; align-items: center; gap: 12px; }
    .disc-live-node { display: flex; flex-direction: column; align-items: center; gap: 4px; text-align: center; }
    .disc-live-icon {
      width: 48px; height: 48px; border-radius: 999px; display: flex; align-items: center; justify-content: center;
      font-size: 18px; font-weight: 700; border: 2px solid rgba(255,255,255,0.45);
      box-shadow: 0 1px 4px rgba(0,0,0,0.18), inset 0 0 0 1px rgba(0,0,0,0.08);
    }
    .disc-live-tier-title { font-size: 12px; font-weight: 700; line-height: 1.2; }
    .disc-live-tier-sub { font-size: 11px; font-weight: 600; line-height: 1.2; opacity: 0.9; }
    .disc-live-bar { height: 6px; border-radius: 999px; overflow: hidden; }
    .disc-live-bar-fill { height: 100%; border-radius: 999px; transition: width 0.2s ease; }
    .disc-live-divider { border: 0; border-top: 1px solid rgba(148, 163, 184, 0.35); margin: 12px 0; }
    .disc-live-hint { font-size: 12px; line-height: 1.45; }
    ...
    @media (max-width: 900px) {
      ...
      .disc-live-widget-row { grid-template-columns: 80px 1fr 80px; gap: 8px; }
    }
  `;
        ...
        <div className="disc-modal-overlay">
          <div className="disc-modal" style={{ maxWidth: 560 }}>
            ...
            <div style={{ marginTop: 12 }} className="disc-info-box">
              ℹ️ Timing follows the discount-level schedule configured in Step 3 of the setup wizard.
            </div>
          </div>
        </div>
      )}
      ...
      {showTierDiscountModal && (
        <div className="disc-modal-overlay">
          <div className="disc-modal">
                ...
                <div className="disc-modal-title">
                  {tierDiscountModalStep === 1
                    ...
                    : tierDiscountModalStep === 2
                      ...
                      : `📅 Schedule — ${tierModalDiscountKey || "discount"}`}
                </div>
                <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>
                  Step {tierDiscountModalStep} of 3
                </div>
              </div>
              <button
            ...
            {/* Step Indicator */}
            <StepIndicator
              step={tierDiscountModalStep}
              total={3}
              labels={["Details", "Tiers", "Schedule"]}
            />

            {/* Status Summary */}
            <div style={{ marginBottom: 24 }}>
              <SetupStatusCard
                step1Label={setupStatusStep1Label}
                step2Label={setupStatusStep2Label}
                step3Label={setupStatusStep3Label}
              />
            </div>
            ...
            {tierDiscountModalStep === 1 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <div className="disc-info-box">
                  ℹ️ Give your discount a unique name, then proceed to add tiers and set a schedule.
                </div>
                <div className="disc-field">
                  ...
                  <input
                    ...
                    className={actionData?.errors?.discountName ? "has-error" : ""}
                    autoComplete="off"
                  />
                  {actionData?.errors?.discountName && (
            ...
            {tierDiscountModalStep === 2 && tierModalDiscountKey && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <button
                  type="button"
                  className="disc-btn disc-btn-back"
                  onClick={() => { setTierDiscountModalStep(1); setTierFormInModalOpen(false); setTierEditId(""); }}
                >
                  ← Back to details
                </button>
                ...
                <div className="disc-info-box">
                  ...
                  Usage counts stay at zero unless order webhooks are approved in the Partner Dashboard.
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 10, paddingTop: 4, borderTop: "1px solid #f1f5f9" }}>
                  {!canProceedToScheduleStep && (
                    <div className="disc-warning-box">
                      ⚠️ Create at least one tier to continue.
                    </div>
                  )}
                  <button
                    type="button"
                    className="disc-btn disc-btn-primary"
                    disabled={!canProceedToScheduleStep}
            ...
            {tierDiscountModalStep === 3 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                {persistedScheduleLocked ? (
                  ...
                  <div className="disc-info-box">
                    📅 Set one shared start and end date-time for this entire discount. Leave both empty for "always on".
                ...
                <div className="disc-advanced-preview-wrap">
                  <div className="disc-advanced-preview">
                    ...
                    >
                      ...
                      <div className="disc-live-widget-row">
                        <div className="disc-live-node">
                          {showTierIcons ? (
                            <div
                              className="disc-live-icon"
                              style={{ background: iconBackgroundColor, color: iconTextColor }}
                            >
                              {tier1Icon || "%"}
                            </div>
                          ) : null}
                          {showTier1Heading && showTierLabels ? (
                            <div className="disc-live-tier-title" style={{ color: tierHeadingColor }}>
                              {tier1LabelText || "Discount"}
                            </div>
                          ) : null}
                          {showTier1Subheading && showTierMinimums ? (
                            <div className="disc-live-tier-sub" style={{ color: tierSubheadingColor }}>
                              {`${minAmountPrefixText || "Min."} Rs. ${(liveTier1Min || 0).toLocaleString("en-IN")}.00`}
                            </div>
                          ) : null}
                        </div>
                        <div className="disc-live-bar" style={{ background: barTrackColor }}>
                          <div
                            className="disc-live-bar-fill"
                            style={{ width: `${liveProgress}%`, background: barFillColor }}
                          />
                        </div>
                        <div className="disc-live-node">
                          {showTierIcons ? (
                          ...
                          {showTier2Heading && showTierLabels ? (
                            <div className="disc-live-tier-title" style={{ color: tierHeadingColor }}>
                              {tier2LabelText || "Free shipping"}
                            </div>
                          ) : null}
                          {showTier2Subheading && showTierMinimums ? (
                            <div className="disc-live-tier-sub" style={{ color: tierSubheadingColor }}>
                              {`${minAmountPrefixText || "Min."} Rs. ${(liveTier2Min || 0).toLocaleString("en-IN")}.00`}
                            </div>
                          ) : null}
                        </div>
                      </div>
                      ...
                      {showHint ? (
                        <div className="disc-live-hint" style={{ color: hintColor }}>
                          {`${subtotalLabel || "Current subtotal"}: Rs. ${livePreviewSubtotal.toLocaleString("en-IN")}.00 (${estimatedShippingLabel || "Estimated shipping"}: Rs. 0.00) - ${liveHintText}`}
                        </div>
                      ) : null}
                    </div>
